...
function* loadAllVehicleDrivers() {
  const vehicleDrivers = yield call([
    vehicleDriverList,
    vehicleDriverList.list,
  ]);
...
function* createVehicleDriver({ payload: { vehicleDriver } }) {
  let result = yield call([vehicleDriverList, vehicleDriverList.insert], {
    vehicleDriver,
  });
  yield put(
    vehicleDriverActions.createVehicleDriverFulfilled(result.vehicleDriver)
  );
}

function* updateVehicleDriver({ payload: { id, changes } }) {
  let result = yield call([vehicleDriverList, vehicleDriverList.update], id, {
    changes,
  });
  yield put(
    vehicleDriverActions.updateVehicleDriverFulfilled(result.vehicleDriver)
  );
}

function* removeVehicleDriver({ payload: { vehicleDriver } }) {
  let result = yield call(
    [vehicleDriverList, vehicleDriverList.remove],
    vehicleDriver.id
  );
...
function* watchLoadVehicleDrivers() {
  yield takeEvery(
    vehicleDriverActions.LOAD_VEHICLE_DRIVERS,
    loadAllVehicleDrivers
  );
}

function* watchCreateVehicleDriver() {
  yield takeEvery(
    vehicleDriverActions.CREATE_VEHICLE_DRIVER,
    createVehicleDriver
  );
}
...
function* watchUpdateVehicleDriver() {
  yield takeEvery(
    vehicleDriverActions.UPDATE_VEHICLE_DRIVER,
    updateVehicleDriver
  );
}
...
function* watchRemoveVehicleDriver() {
  yield takeEvery(
    vehicleDriverActions.REMOVE_VEHICLE_DRIVER,
    removeVehicleDriver
  );
}
