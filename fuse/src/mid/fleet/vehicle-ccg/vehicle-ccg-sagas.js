...
function* createVehicleCcg({ payload: { vehicleCcg } }) {
	const { vehicleCcg: newCcg } = yield call([vehicleCcgList, vehicleCcgList.insert], vehicleCcg);
	yield put(vehicleCcgActions.createVehicleCcgFulfilled(newCcg));
}

function* updateVehicleCcg({ payload: { vehicleCcg, changes } }) {
	console.log(changes);
	const result = yield call([vehicleCcgList, vehicleCcgList.update], vehicleCcg.id, {
		vehicleCcg
	});
...
function* removeVehicleCcg({ payload: { vehicleCcg } }) {
	const { id } = yield call([vehicleCcgList, vehicleCcgList.remove], vehicleCcg.id);
...
function* watchUpdateVehicleCcgBatch() {
	yield takeEvery(vehicleCcgActions.UPDATE_VEHICLE_CCG_ARRAY, updateVehicleCcgArray);
}
