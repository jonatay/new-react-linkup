...
export const getVisibleVehicles = createSelector(
	...
	(vehicleList, filter, showInactive, vehicleCcgs) =>
		vehicleList
			.filter(
				vehicle =>
					((!showInactive && vehicle.is_active) || (showInactive && !vehicle.is_active)) &&
			...
			.map(v => ({
				...v,
				vehicleCcgs: vehicleCcgs.filter(vccg => vccg.vehicle_id === v.id).toArray()
			}))
);

export const getVehicleById = createSelector(getVehicleList, (vehicleList, id) =>
	vehicleList.filter(vehicle => vehicle.id === id)
);

export const getVehiclesList = createSelector(getVehicleList, vehicleList => vehicleList);

export const getVehicles = createSelector(getVehiclesList, vehicleList => vehicleList.toArray());
