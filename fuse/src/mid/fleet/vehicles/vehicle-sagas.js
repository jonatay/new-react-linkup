...
function* toggleVehicleIsActive({ payload }) {
	const result = yield call([vehicleList, vehicleList.remove], payload.vehicle.id);
	yield put(vehicleActions.toggleVehicleIsActiveFulfilled(result.vehicle));
}

function* watchToggleVehicleIsActive() {
	yield takeEvery(vehicleActions.TOGGLE_VEHICLE_IS_ACTIVE, toggleVehicleIsActive);
}
...
function* updateVehicle({
	...
	const result = yield call([vehicleList, vehicleList.update], vehicle.id, {
		changes
	});
