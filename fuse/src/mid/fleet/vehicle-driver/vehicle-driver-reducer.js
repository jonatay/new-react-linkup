...
export function vehicleDriverReducer(state = new VehicleDriverState(), { payload, type }) {
	switch (type) {
		...
		case vehicleDriverActions.CREATE_VEHICLE_DRIVER_FULFILLED:
			return state.set('list', state.list.unshift(payload.vehicleDriver));

		case vehicleDriverActions.UPDATE_VEHICLE_DRIVER_FULFILLED:
			return state.set(
				'list',
				state.list.map(r => {
					return r.id === payload.vehicleDriver.id ? payload.vehicleDriver : r;
				})
			);
		...
		case vehicleDriverActions.LOAD_VEHICLE_DRIVERS_FULFILLED:
			return state.set('list', new List(payload.vehicleDrivers));
