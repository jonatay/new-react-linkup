	...
	(list, { filtered }) =>
		!filtered
			...
			: // eslint-disable-next-line no-unused-vars
			  list.filter((val, key) => filtered.reduce((r, v, k) => (!r ? r : val[v.id] === v.value), true))
);
...
export const getVisibleFleetTransactions = createSelector(
	...
	getFleetTransactionFilter,
	// eslint-disable-next-line no-unused-vars
	(list, { page, pageSize, sorted, filtered }) => list.toArray().slice(page * pageSize, page * pageSize + pageSize)
);

export const getFleetTransactionById = createSelector(getFleetTransactionList, (fleetTransactionList, id) =>
...
export const getListOptions = createSelector(getListOptionsFromState, listOptions => listOptions);

const getPerId = ft =>
	ft.tax_year === 0
		? 'Current'
		: // eslint-disable-next-line prefer-template
		  `${String('0000' + ft.tax_month <= 2 ? ft.tax_year : ft.tax_year - 1).slice(-4)}${String(
				// eslint-disable-next-line prefer-template
				'00' + ft.tax_month
		  ).slice(-2)}`;

export const getFleetTransactionPeriods = createSelector(getFleetTransactionList, fleetTransactionList =>
	fleetTransactionList
		.toArray()
		.reduce((ret, ft) => (ret.find(p => p === getPerId(ft)) ? ret : [...ret, getPerId(ft)]), [])
		.sort((a, b) => (a > b ? 1 : a < b ? -1 : 0))
);

export const getFleetTransactionTypes = createSelector(getFleetTransactionList, fleetTransactionList =>
	fleetTransactionList
		.toArray()
		.reduce((ret, ft) => (ret.find(p => p === ft.transaction_type) ? ret : [...ret, ft.transaction_type]), [])
		.sort((a, b) => (a.localeCompare(b) < 0 ? 1 : a.localeCompare(b) > 0 ? -1 : 0))
);
...
export const getFleetTransactionSummary = createSelector(
	...
	(fleetTransactionList, fleetTransactionPeriods, fleetTransactionTypes) =>
		fleetTransactionList.toArray().reduce((vehSumm, ft) => {
			...
			const tranType = per.types[ft.transaction_type]
