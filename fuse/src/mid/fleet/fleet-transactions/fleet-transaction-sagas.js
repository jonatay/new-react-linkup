...
function* loadAllFleetTransactions({ payload }) {
	const fleetTransactions = yield call([fleetTransactionList, fleetTransactionList.list], payload.listParams);
...
function* updateFleetTransaction({ payload }) {
	const { changes } = payload; // jsondiffpatch.diff(payload.fleetTransaction, payload.changes);
	const result = yield call([fleetTransactionList, fleetTransactionList.update], payload.fleetTransaction.id, {
		changes
	});
...
function* toggleFleetTransactionIsActive({ payload }) {
	const result = yield call([fleetTransactionList, fleetTransactionList.remove], payload.fleetTransaction.id);
...
function* watchToggleFleetTransactionIsActive() {
	yield takeEvery(fleetTransactionActions.TOGGLE_FLEET_TRANSACTION_IS_ACTIVE, toggleFleetTransactionIsActive);
}
