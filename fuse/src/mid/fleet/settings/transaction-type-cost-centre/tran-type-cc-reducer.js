...
export function tranTypeCcReducer(state = new TranTypeCcState(), { payload, type }) {
	switch (type) {
		case tranTypeCcActions.CREATE_TRANSACTION_TYPE_COST_CENTRE_FULFILLED:
			return state.set('list', state.list.unshift(payload.tranTypeCc));

		case tranTypeCcActions.UPDATE_TRANSACTION_TYPE_COST_CENTRE_FULFILLED:
			return state.set(
				'list',
				state.list.map(r => {
					return r.id === payload.tranTypeCc.id ? payload.tranTypeCc : r;
				})
			);
		...
		case tranTypeCcActions.LOAD_TRANSACTION_TYPE_COST_CENTRES_FULFILLED:
			return state.set('list', new List(payload.tranTypeCcs));
