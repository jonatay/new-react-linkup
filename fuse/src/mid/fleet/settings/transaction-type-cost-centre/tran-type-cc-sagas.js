...
function* loadAllTranTypeCcs() {
	const tranTypeCcs = yield call([tranTypeCcList, tranTypeCcList.list]);
	yield put(tranTypeCcActions.loadTranTypeCcsFulfilled(tranTypeCcs));
}

function* createTranTypeCc({ payload: { tranTypeCc } }) {
	const result = yield call([tranTypeCcList, tranTypeCcList.insert], {
		transactionTypeCostCentre: tranTypeCc
	});
	yield put(tranTypeCcActions.createTranTypeCcFulfilled(result.transactionTypeCostCentre));
}

function* updateTranTypeCc({ payload: { id, changes } }) {
	const result = yield call([tranTypeCcList, tranTypeCcList.update], id, { changes });
	yield put(tranTypeCcActions.updateTranTypeCcFulfilled(result.tranTypeCc));
}

function* removeTranTypeCc({ payload: { tranTypeCc } }) {
	const result = yield call([tranTypeCcList, tranTypeCcList.remove], tranTypeCc.id);
	if (result.status === 'deleted') {
		yield put(tranTypeCcActions.removeTranTypeCcFulfilled(tranTypeCc));
	} else {
...
function* watchLoadTranTypeCcs() {
	yield takeEvery(tranTypeCcActions.LOAD_TRANSACTION_TYPE_COST_CENTRES, loadAllTranTypeCcs);
}

function* watchCreateTranTypeCc() {
	yield takeEvery(tranTypeCcActions.CREATE_TRANSACTION_TYPE_COST_CENTRE, createTranTypeCc);
}

function* watchUpdateTranTypeCc() {
	yield takeEvery(tranTypeCcActions.UPDATE_TRANSACTION_TYPE_COST_CENTRE, updateTranTypeCc);
}

function* watchRemoveTranTypeCc() {
	yield takeEvery(tranTypeCcActions.REMOVE_TRANSACTION_TYPE_COST_CENTRE, removeTranTypeCc);
}

function* watchUpdateTranCcBatch() {
	yield takeEvery(tranTypeCcActions.UPDATE_TRANSACTION_TYPE_COST_CENTRE_BATCH, updateTranTypeCcBatch);
}
