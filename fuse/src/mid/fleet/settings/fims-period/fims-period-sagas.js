...
import { call, fork, put, take, takeEvery, race, cancel } from 'redux-saga/effects';
...
function* loadFimsPeriods() {
	const { status, fimsPeriods, error } = yield call([fimsPeriodList, fimsPeriodList.list]);
	if (status === 'ok') {
...
function* postFimsBatch({ payload }) {
	...
	const postFimsBatchResult = yield call([fimsPeriodList, fimsPeriodList.postFimsBatch], { fimsBatch });
	yield put(fimsPeriodActions.postFimsBatchFulfilled(postFimsBatchResult));
}

function* importFimsPeriod({ payload: { id } }) {
	// console.log(payload);
	const { status, fimsPeriod, error } = yield call([fimsPeriodList, fimsPeriodList.importFimsPeriod], id);
	if (status === 'ok') {
...
function* watchImportFimsPeriodBatch() {
	yield takeEvery(fimsPeriodActions.IMPORT_FIMS_PERIOD_BATCH, importFimsPeriodBatch);
}
