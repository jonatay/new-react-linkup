...
export function fimsPeriodReducer(state = new FimsPeriodState(), { payload, type }) {
	switch (type) {
			...
			return state.set('isAvailable', true).set(
				...
				state.list.find(rec => rec.id === payload.fimsPeriod.id)
					? state.list.map(fimsPeriod => {
							return fimsPeriod.id === payload.fimsPeriod.id ? payload.fimsPeriod : fimsPeriod;
					  })
		...
		case fimsPeriodActions.REMOVE_FIRMS_PERIOD_FULFILLED:
			return state.set(
				'list',
				state.list.filter(fp => fp.id !== payload.id)
			);
