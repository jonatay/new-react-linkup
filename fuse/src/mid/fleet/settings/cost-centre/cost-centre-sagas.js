...
import { tranTypeCcActions, transactionTypeActions, costCentreGroupActions } from '../index';
...
function* createCostCentre({
	payload: {
		costCentre: { transactionTypes, ...costCentre }
	}
}) {
	const { costCentre: newCostCentre } = yield call([costCentreList, costCentreList.insert], { costCentre });
	const ttcc = transactionTypes.map(tt => ({
		...tt,
		id: null,
...
function* updateCostCentre({
	...
	if (JSON.stringify(costCentre) !== JSON.stringify(changes)) {
		const result = yield call([costCentreList, costCentreList.update], costCentre.id, {
			costCentre,
			changes
		});
...
function* removeCostCentre({ payload: { id } }) {
	const result = yield call([costCentreList, costCentreList.remove], id);
