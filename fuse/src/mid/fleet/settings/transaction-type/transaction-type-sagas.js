...
function* loadAllTransactionTypes() {
	const transactionTypes = yield call([transactionTypeList, transactionTypeList.list]);
	yield put(transactionTypeActions.loadTransactionTypesFulfilled(transactionTypes));
}
...
function* watchLoadTransactionTypes() {
	yield takeEvery(transactionTypeActions.LOAD_TRANSACTION_TYPES, loadAllTransactionTypes);
}
