...
export const getAttendLogById = createSelector(getAttendLogList, (list, id) => list.filter(rec => rec.id === id));

export const getAttendLogs = createSelector(getAttendLogList, list => list.toArray());

export const getAttendLogWithUserDept = createSelector(getAttendLogs, getAttendUsersWithDept, (logs, users) =>
	logs.map(log => ({
		...log,
...
export const getAttendLogFiltered = createSelector(getAttendLogList, getAttendLogFilter, (attendLogList, filter) =>
	attendLogList.filter(logItem => true)
);
	...
	(logs, users, depts) =>
		unflatten(
			depts.map(dept => ({
				...dept,
				users: users
					.filter(user => user.dept_id === dept.id)
					.map(user => ({
						...user,
...
const getAttendForUser = (user, logs) =>
	logs
		...
		.reduce((ret, log) => {
			...
			const idx = ret.findIndex(e => e.log_time.slice(0, 10) === log.log_time.slice(0, 10));
			if (idx === -1) {
				return [...ret, { log_time: log.log_time, entry_time: log.log_time }];
			}
			return [...ret.slice(0, idx), { ...ret[idx], exit_time: log.log_time }, ...ret.slice(idx + 1)];
		}, [])
		.reverse();

export const getAttendUsersWithDeptLog = createSelector(getAttendUsersWithDept, getAttendLogs, (users, logs) =>
	users
		.toArray()
...
export const getAttendLogsPeriods = createSelector(getAttendLogList, getAttendLogFilter, (logs, filter) =>
	logs
		.toArray()
		...
		.filter(log => (filter.excludeWeekends ? moment(log).day() > 0 && moment(log).day() < 6 : true))
);
...
export const getAttendLogTableData = createSelector(
	...
	(users, logs, periods, filter) =>
		users
			.filter(user => (filter.depts && filter.depts.length > 0 ? isUserInDepts(user, filter.depts) : false))
			.map(user => ({
				...user,
				...
				...periods.reduce(
					(r, p) => ({
						...r,
						[p]: logs.filter(log => log.user_id === user.id && log.log_time.slice(0, 10) === p)
					}),
					{}
				)
			}))
