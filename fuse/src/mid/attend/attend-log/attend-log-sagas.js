...
function* loadAllAttendLogs({ payload }) {
	const attendLogs = yield call([attendLogList, attendLogList.list], payload.listParams);
	yield put(attendLogActions.loadAttendLogsFulfilled(attendLogs));
}
function* createAttendLog({ payload: { attendLog } }) {
	const result = yield call([attendLogList, attendLogList.insert], {
		attendLog
	});
...
function* updateAttendLog({ payload: { id, changes } }) {
	const result = yield call([attendLogList, attendLogList.update], id, {
		changes
	});
...
function* removeAttendLog({ payload: { attendLog } }) {
	const result = yield call([attendLogList, attendLogList.remove], attendLog.id);
...
function* loadAttendLogPdf({ payload: { listParams } }) {
	const result = yield call([attendLogList, attendLogList.getPdf], { listParams }, 'list-pdf');
