...
function* createAttendDept({ payload: { attendDept } }) {
	const result = yield call([attendDeptList, attendDeptList.insert], {
		attendDept
	});
...
function* updateAttendDept({ payload: { id, changes } }) {
	const result = yield call([attendDeptList, attendDeptList.update], id, {
		changes
	});
...
function* removeAttendDept({ payload: { attendDept } }) {
	const result = yield call([attendDeptList, attendDeptList.remove], attendDept.id);
