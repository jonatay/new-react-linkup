...
export const getAttendDeptById = createSelector(getAttendDeptList, (list, id) => list.filter(rec => rec.id === id));

export const getAttendDepts = createSelector(getAttendDeptList, list => list.toArray());
...
export const getAttendBranchesTree = createSelector(getAttendDepts, depts =>
	unflatten(
		depts.map(dept => ({
			title: dept.name,
...
export const getAttendDeptsTree = createSelector(getAttendDepts, Depts, (aDepts, depts) =>
	// console.log(aDepts) ||
	unflatten(
		depts.reduce(
			(items, dept) => [
