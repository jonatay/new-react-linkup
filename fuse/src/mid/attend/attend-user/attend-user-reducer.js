...
export function attendUserReducer(state = new AttendUserState(), { payload, type }) {
	switch (type) {
		case attendUserActions.CREATE_ATTEND_USER_FULFILLED:
			return state.set('list', state.list.unshift(payload.attendUser));

		case attendUserActions.UPDATE_ATTEND_USER_FULFILLED:
			return state.set(
				'list',
				state.list.map(r => {
					return r.id === payload.attendUser.id ? payload.attendUser : r;
				})
			);
		...
		case attendUserActions.LOAD_ATTEND_USERS_FULFILLED:
			return state.set('list', new List(payload.attendUsers));
