...
function* createAttendUser({ payload: { attendUser } }) {
	const result = yield call([attendUserList, attendUserList.insert], {
		attendUser
	});
...
function* updateAttendUser({ payload: { id, changes } }) {
	const result = yield call([attendUserList, attendUserList.update], id, {
		changes
	});
...
function* removeAttendUser({ payload: { attendUser } }) {
	const result = yield call([attendUserList, attendUserList.remove], attendUser.id);
