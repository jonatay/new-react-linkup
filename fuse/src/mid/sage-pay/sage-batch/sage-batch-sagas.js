...
function* querySageBatch({ payload: { id } }) {
	const { sageBatch } = yield call([sageBatchList, sageBatchList.querySageBatch], id);
	yield put(sageBatchActions.querySageBatchFulfilled(sageBatch));
}

function* submitSageBatch({ payload: { id } }) {
	const { sageBatch } = yield call([sageBatchList, sageBatchList.submitBatch], id);
...
function* createSageBatch({ payload: { params } }) {
	const { sageBatch } = yield call([sageBatchList, sageBatchList.createSageBatch], { params });
