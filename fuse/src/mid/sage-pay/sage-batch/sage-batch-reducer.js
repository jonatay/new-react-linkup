...
export function sageBatchReducer(state = new SageBatchState(), { payload, type }) {
	switch (type) {
		...
		case sageBatchActions.QUERY_SAGE_BATCH_FULFILLED:
			return state.set(
				'list',
				state.list.map(sageBatch => (sageBatch.id === payload.sageBatch.id ? payload.sageBatch : sageBatch))
			);

		case sageBatchActions.DELETE_SAGE_BATCH_FULFILLED:
			return state.set(
				'list',
				state.list.filter(r => r.id !== payload.id)
			);
