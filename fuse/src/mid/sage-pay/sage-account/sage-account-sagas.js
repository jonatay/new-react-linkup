...
function* loadSageEmpAccounts() {
	yield put(sageBankActions.loadSageBanks());
	yield put(sageBBranchActions.loadSageBBranches());
	const { sageAccounts } = yield call([sageAccountList, sageAccountList.listEmp]);
	yield put(sageAccountActions.loadSageAccountsFulfilled(sageAccounts));
}

function* importBestAccounts({ payload: { bestCreditors } }) {
	const { sageAccounts } = yield call([sageAccountList, sageAccountList.importBest], { bestCreditors });
	yield put(sageAccountActions.importBestAccountsFulfilled(sageAccounts));
}

function* importCubitAccounts() {
	const { sageAccounts } = yield call([sageAccountList, sageAccountList.importCubit]);
	yield put(sageAccountActions.importCubitAccountsFulfilled(sageAccounts));
}

function* validateSageAccount({ payload: { id } }) {
	const { sageAccount, validationResult } = yield call([sageAccountList, sageAccountList.validateSageAccount], id);
	yield put(sageAccountActions.validateSageAccountFulfilled(sageAccount, validationResult));
}
...
function* watchLoadSageEmpAccounts() {
	yield takeEvery(sageAccountActions.LOAD_SAGE_EMP_ACCOUNTS, loadSageEmpAccounts);
}
...
function* watchImportCubitAccounts() {
	yield takeEvery(sageAccountActions.IMPORT_CUBIT_ACCOUNTS, importCubitAccounts);
}

function* watchValidateSageAccount() {
	yield takeEvery(sageAccountActions.VALIDATE_SAGE_ACCOUNT, validateSageAccount);
}
