...
export function sageAccountReducer(state = new SageAccountState(), { payload, type }) {
	switch (type) {
		...
		case sageAccountActions.UPDATE_SAGE_ACCOUNT_FULFILLED: {
			return state.set(
				'list',
				state.list.map(sageAccount =>
					sageAccount.id === payload.sageAccount.id ? payload.sageAccount : sageAccount
				)
			);
		}
