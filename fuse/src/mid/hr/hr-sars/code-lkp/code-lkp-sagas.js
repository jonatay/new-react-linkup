...
function* loadAllCodeLkps() {
	const { codeLkps } = yield call([codeLkpList, codeLkpList.list]);
	yield put(codeLkpActions.loadCodeLkpsFulfilled(codeLkps));
}
function* createCodeLkp({ payload: { codeLkp } }) {
	const result = yield call([codeLkpList, codeLkpList.insert], {
		codeLkp
	});
	yield put(codeLkpActions.createCodeLkpFulfilled(result.codeLkp));
}

function* updateCodeLkp({ payload: { id, changes } }) {
	const result = yield call([codeLkpList, codeLkpList.update], id, { changes });
	yield put(codeLkpActions.updateCodeLkpFulfilled(result.codeLkp));
}

function* removeCodeLkp({ payload: { codeLkp } }) {
	const result = yield call([codeLkpList, codeLkpList.remove], codeLkp.id);
	if (result.status === 'deleted') {
		yield put(codeLkpActions.removeCodeLkpFulfilled(codeLkp));
	} else {
...
function* watchLoadCodeLkps() {
	yield takeEvery(codeLkpActions.LOAD_CODE_LKPS, loadAllCodeLkps);
}

function* watchCreateCodeLkp() {
	yield takeEvery(codeLkpActions.CREATE_CODE_LKP, createCodeLkp);
}

function* watchUpdateCodeLkp() {
	yield takeEvery(codeLkpActions.UPDATE_CODE_LKP, updateCodeLkp);
}

function* watchRemoveCodeLkp() {
	yield takeEvery(codeLkpActions.REMOVE_CODE_LKP, removeCodeLkp);
}
