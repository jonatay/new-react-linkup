...
function* loadAllEmpCodes() {
	const { empCodes } = yield call([empCodeList, empCodeList.list]);
	yield put(empCodeActions.loadEmpCodesFulfilled(empCodes));
}
function* createEmpCode({ payload: { empCode } }) {
	const result = yield call([empCodeList, empCodeList.insert], {
		empCode
	});
	yield put(empCodeActions.createEmpCodeFulfilled(result.empCode));
}

function* updateEmpCode({ payload: { id, changes } }) {
	const result = yield call([empCodeList, empCodeList.update], id, { changes });
	yield put(empCodeActions.updateEmpCodeFulfilled(result.empCode));
}

function* removeEmpCode({ payload: { empCode } }) {
	const result = yield call([empCodeList, empCodeList.remove], empCode.id);
	if (result.status === 'deleted') {
		yield put(empCodeActions.removeEmpCodeFulfilled(empCode));
	} else {
...
function* watchLoadEmpCodes() {
	yield takeEvery(empCodeActions.LOAD_EMP_CODES, loadAllEmpCodes);
}

function* watchCreateEmpCode() {
	yield takeEvery(empCodeActions.CREATE_EMP_CODE, createEmpCode);
}

function* watchUpdateEmpCode() {
	yield takeEvery(empCodeActions.UPDATE_EMP_CODE, updateEmpCode);
}

function* watchRemoveEmpCode() {
	yield takeEvery(empCodeActions.REMOVE_EMP_CODE, removeEmpCode);
}
