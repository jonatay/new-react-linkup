...
export function payPointReducer(state = new PayPointState(), { payload, type }) {
	switch (type) {
		case payPointActions.CREATE_PAY_POINT_FULFILLED:
			return state.set('list', state.list.unshift(payload.payPoint));

		case payPointActions.UPDATE_PAY_POINT_FULFILLED:
			return state.set(
				'list',
				state.list.map(r => {
					return r.id === payload.payPoint.id ? payload.payPoint : r;
				})
			);
		...
		case payPointActions.LOAD_PAY_POINTS_FULFILLED:
			return state.set('list', new List(payload.payPoints));
