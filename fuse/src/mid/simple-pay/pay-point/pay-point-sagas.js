...
function* createPayPoint({ payload: { payPoint } }) {
	const result = yield call([payPointList, payPointList.insert], {
		payPoint
	});
...
function* updatePayPoint({ payload: { id, changes } }) {
	const result = yield call([payPointList, payPointList.update], id, { changes });
	yield put(payPointActions.updatePayPointFulfilled(result.payPoint));
}

function* removePayPoint({ payload: { payPoint } }) {
	const result = yield call([payPointList, payPointList.remove], payPoint.id);
