...
function* loadAllSpEmployees() {
	const { spEmployees } = yield call([spEmployeeList, spEmployeeList.list]);
	yield put(spEmployeeActions.loadSpEmployeesFulfilled(spEmployees));
}
function* createSpEmployee({ payload: { spEmployee } }) {
	const result = yield call([spEmployeeList, spEmployeeList.insert], {
		spEmployee
	});
	yield put(spEmployeeActions.createSpEmployeeFulfilled(result.spEmployee));
}

function* updateSpEmployee({ payload: { id, changes } }) {
	const result = yield call([spEmployeeList, spEmployeeList.update], id, {
		changes
	});
	yield put(spEmployeeActions.updateSpEmployeeFulfilled(result.spEmployee));
}

function* removeSpEmployee({ payload: { spEmployee } }) {
	const result = yield call([spEmployeeList, spEmployeeList.remove], spEmployee.id);
	if (result.status === 'deleted') {
		yield put(spEmployeeActions.removeSpEmployeeFulfilled(spEmployee));
	} else {
...
function* watchLoadSpEmployees() {
	yield takeEvery(spEmployeeActions.LOAD_SP_EMPLOYEES, loadAllSpEmployees);
}

function* watchCreateSpEmployee() {
	yield takeEvery(spEmployeeActions.CREATE_SP_EMPLOYEE, createSpEmployee);
}

function* watchUpdateSpEmployee() {
	yield takeEvery(spEmployeeActions.UPDATE_SP_EMPLOYEE, updateSpEmployee);
}

function* watchRemoveSpEmployee() {
	yield takeEvery(spEmployeeActions.REMOVE_SP_EMPLOYEE, removeSpEmployee);
}
