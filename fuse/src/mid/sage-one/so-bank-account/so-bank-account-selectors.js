...
export const getSoBankAccountById = createSelector(getSoBankAccountList, (list, id) =>
	list.filter(rec => rec.id === id)
);

export const getSoBankAccounts = createSelector(getSoBankAccountList, list => list.toArray());
...
export const getSoBankAccountsFiltered = createSelector(
	...
	(bankAccounts, filter) =>
		bankAccounts
			...
			.map(ba => ({
				...ba,
				PaymentMethodLkp: paymentMethod.find(pm => pm.id === ba.PaymentMethod).name,
