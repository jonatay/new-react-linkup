...
function* loadAllSoBankAccounts() {
	const { soBankAccounts } = yield call([soBankAccountList, soBankAccountList.list]);
...
function* createSoBankAccount({ payload: { soBankAccount } }) {
	const result = yield call([soBankAccountList, soBankAccountList.insert], {
		soBankAccount
	});
	yield put(soBankAccountActions.createSoBankAccountFulfilled(result.soBankAccount));
}

function* updateSoBankAccount({ payload: { id, changes } }) {
	const result = yield call([soBankAccountList, soBankAccountList.update], id, {
		changes
	});
	yield put(soBankAccountActions.updateSoBankAccountFulfilled(result.soBankAccount));
}

function* removeSoBankAccount({ payload: { soBankAccount } }) {
	const result = yield call([soBankAccountList, soBankAccountList.remove], soBankAccount.id);
...
function* watchLoadSoBankAccounts() {
	yield takeEvery(soBankAccountActions.LOAD_SO_BANK_ACCOUNTS, loadAllSoBankAccounts);
}

function* watchCreateSoBankAccount() {
	yield takeEvery(soBankAccountActions.CREATE_SO_BANK_ACCOUNT, createSoBankAccount);
}

function* watchUpdateSoBankAccount() {
	yield takeEvery(soBankAccountActions.UPDATE_SO_BANK_ACCOUNT, updateSoBankAccount);
}

function* watchRemoveSoBankAccount() {
	yield takeEvery(soBankAccountActions.REMOVE_SO_BANK_ACCOUNT, removeSoBankAccount);
}
