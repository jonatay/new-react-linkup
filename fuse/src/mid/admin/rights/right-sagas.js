...
import { navActions } from '../../admin/users';
...
const createRight = write.bind(null, rightList, rightList.push, rightActions.createRightFailed);
const removeRight = write.bind(null, rightList, rightList.remove, rightActions.removeRightFailed);
const updateRight = write.bind(null, rightList, rightList.update, rightActions.updateRightFailed);
...
function* watchCreateRight() {
	while (true) {
		const { payload } = yield take(rightActions.CREATE_RIGHT);
...
function* watchRemoveRight() {
	while (true) {
		const { payload } = yield take(rightActions.REMOVE_RIGHT);
...
function* watchUpdateRight() {
	while (true) {
		const { payload } = yield take(rightActions.UPDATE_RIGHT);
