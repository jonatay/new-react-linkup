...
import { call, fork, put, take, cancel, takeEvery, delay } from 'redux-saga/effects';
...
function* signIn(authProvider) {
	...
	// 	const idToken = yield call([authData.user, authData.user.getIdToken]);
	// 	const customClaim = JSON.parse(b64DecodeUnicode(idToken.split('.')[1]));
	// 	yield put(authActions.signInFulfilled(authData.user, idToken, customClaim.roles));
	// 	yield history.push('/');
	// } catch (error) {
...
function* registerNewUser({ payload }) {
	...
	// 		[firebaseAuth, firebaseAuth.createUserAndRetrieveDataWithEmailAndPassword],
	// 		email,
	// 		password
	// 	);
...
function* watchSignInWithEmailPassword() {
	yield takeEvery(authActions.SIGN_IN_WITH_EMAIL_PASSWORD, signInWithEmailPassword);
}
...
function* watchRefreshIdToken() {
	while (true) {
		...
		const idToken = yield call([payload.authUser, payload.authUser.getIdToken], true);
