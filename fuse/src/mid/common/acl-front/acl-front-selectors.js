...
export const getAclRoles = createSelector(getAcl, getUid, (acl, uid) => {
	if (typeof acl === 'object' && typeof acl.users === 'object' && typeof acl.users[uid] === 'object') {
		const pRoles = acl.parents;
		const roles = Object.keys(acl.users[uid]);
		return _.flatten(
			roles.concat(
				roles
					.map(r => Object.keys(pRoles).map(p => (p === r ? Object.keys(pRoles[p]) : null)))
...
const extractFront = (node, roles) => {
	const ret = [];
	for (const r of Object.keys(node)) {
		if (roles.indexOf(r) >= 0 && typeof node[r].front === 'object') {
			const { front } = node[r];
			for (const res of Object.keys(front)) {
				const fEle = ret.find(f => f.resource === res);
				if (fEle) {
...
export const getAclFront = createSelector(getAcl, getAclRoles, (acl, roles) => {
	if (typeof acl === 'object' && typeof acl.resources === 'object' && typeof roles === 'object') {
...
const childNode = (title, obj, depth = 1) => {
	return {
		...
		...(Object.keys(obj).length > 0
			? {
					children: Object.keys(obj).map(key => childNode(key, obj[key], depth + 1))
			  }
			: {})
	};
};
...
export const getRoles = createSelector(getAcl, acl => {
	return typeof acl.meta === 'object' && typeof acl.meta.roles === 'object' ? Object.keys(acl.meta.roles) : [];
});
...
export const getResources = createSelector(getAcl, acl => {
	return typeof acl.resources === 'object' ? extractResources(acl.resources) : [];
});
